...
class PosOrder(models.TransientModel):
    ...
    def daily_report(self):
        ...
        selected_date2 = datetime.strptime(selected_date, '%Y-%m-%d %H:%M:%S')
        last_day_selected_month = date(selected_date2.year, selected_date2.month, 27)
        ...
        if self.report_date == str(last_day_selected_month):
            data2 = {'date_start': str(month_start), 'date_stop': str(today_end), 'user_id': False,
                        'staff_assigned_id': False, 'partner_id': False}
