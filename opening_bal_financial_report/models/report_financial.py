    ...
    def _compute_account_balance(self, accounts):
        ...
        for key, value in res.items():
            ...
            request = "SELECT account_id as id, " + ', '.join(mapping.values()) + \
                      ...
                      + filters + \
                      " GROUP BY account_id"
            params = (tuple(accounts._ids),) + tuple(init_where_params)
            self.env.cr.execute(request, params)
            for row in self.env.cr.dictfetchall():
                result_opening[row['id']] = row
            for res_open_key, res_open_value in result_opening.items():
                if key == res_open_key:
                    value['opening_balance'] = res_open_value['balance']
            # Opening balance......
        return res
            ...
            if res[report.id].get('account'):
                ...
                for account_id, value in res[report.id]['account'].items():
                    ...
                    vals = {
                        ...
                        'opening_balance': value['opening_balance']* report.sign or 0.0,
                        'type': 'account',
