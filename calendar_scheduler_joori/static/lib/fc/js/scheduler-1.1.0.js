...
ResourceDayTableMixin = {
  ...
  renderHeadResourceCellHtml: function(resource, colspan) {
    ...
    var img_src = '<img class="o_cal_avatar" src="/web/image/hr.employee/emp_id/image_small" style="border-radius: 50%;"></br>';
    return '<th class="fc-resource-cell" ' + (colspan > 1 ? ' colspan="' + colspan + '"' : '') + '>'+ img_src.replace("emp_id", resource.id) + htmlEscape(this.view.getResourceText(resource)) + '</th>';
  },
