...
class pos_staff_oreder_line(models.Model):
    ...
    staff_assigned_id = fields.Many2one('hr.employee', string='Employee',domain=[('is_beautician','=',True)])
    ...
    def default_get(self, fields):
        res = super(pos_staff_oreder_line, self).default_get(fields)
        staff_assigned_id = False
        if self._context:
            context_keys = self._context.keys()
            if 'lines' in context_keys:
                for line in self._context.get('lines'):
                    if line[2]:
                        staff_assigned_id =  line[2].get('staff_assigned_id')
        res.update({'staff_assigned_id': staff_assigned_id})
        return res
...
class pos_details_report(models.TransientModel):
    ...
    def email_report2(self):
        ...
        data = {'date_start': self.start_date, 'date_stop': self.end_date, 'config_ids': self.pos_config_ids.ids,
                'staff_assigned_id': self.staff_assigned_id.id, 'partner_id': self.partner_id.id, 'user_id':self.user_id.id}
