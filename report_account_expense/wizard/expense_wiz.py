...
class ExpenseReportWizard(models.TransientModel):
    ...
    def _domain_account_id(self):
        direct_exp = self.env.ref('account.data_account_type_expenses')
        indirect_exp = self.env.ref('add_change_account_type.data_account_type_other_expenses')
        domain = [('user_type_id', 'in', (direct_exp.id, indirect_exp.id))]
        return domain
    ...
    def print_expense_report(self):
        ...
        return self.env['report'].get_action([], 'report_account_expense.expense_report_pdf', data=data)


class ReportAdvPayment(models.AbstractModel):
    _name = 'report.report_account_expense.expense_report_pdf'
    ...
    @api.model
    def get_expense_rep_details(self, period_start=False, period_stop=False, partner_id=False, account_id=False):
        direct_exp = self.env.ref('account.data_account_type_expenses')
        indirect_exp = self.env.ref('add_change_account_type.data_account_type_other_expenses')
        dom = [
            ('move_id.state', '=', 'posted'),
            ('account_id.user_type_id', 'in', (direct_exp.id, indirect_exp.id)),
        ]
        if period_stop:
        ...
        expense_records = self.env['account.move.line'].search(dom,)
        debit_sum = 0.0
        credit_sum = 0.0
        debit_opening = 0.0
        credit_opening = 0.0
        ...
        current_balance = 0.0
        for expense_rec in expense_records:
            debit_sum += expense_rec.debit
            credit_sum += expense_rec.credit
            current_balance += (expense_rec.debit - expense_rec.credit)
        closing_balance = opening_balance + current_balance
        return {
            'expense_records': sorted(expense_records, key=lambda l: l.date),
    ...
    def render_html(self, docids, data=None):
        data = dict(data or {})
        data.update(self.get_expense_rep_details(data['period_start'],
                                                 data['period_stop'],
                                                 data['partner_id'],
                                                 data['account_id'],
                                                 ))
        return self.env['report'].render('report_account_expense.expense_report_pdf', data)
