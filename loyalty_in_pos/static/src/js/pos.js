odoo.define('loyalty_in_pos.loyalty_program', function (require){
"use strict"
    ...
    models.Orderline = models.Orderline.extend({
        get_reward: function(){
            console.log("get_reqord of line===", this.pos.loyalty.rewards_by_id[this.reward_id])
    ...
    models.Order = models.Order.extend({
        ...
        has_discount_reward: function(){
            var res = false;
            var lines = this.get_orderlines();
            console.log("has discount funstionnn")

            for (var i = 0; i < lines.length; i++) {
                var line = lines[i];
                var reward = line.get_reward();
                console.log("reward of line==", reward);
                if (reward && reward.type === 'discount') {
                    console.log("reward and reward type = discount res true")
                    res = true;
                    break;
                }
            }
        ...
        has_gift_reward: function(){
            var res = false;
            var lines = this.get_orderlines();
            console.log("has gift funstionnn")

            for (var i = 0; i < lines.length; i++) {
                var line = lines[i];
                var reward = line.get_reward();
                console.log("reward of line==", reward);
                if (reward && reward.type === 'gift') {
                    console.log("reward and reward type = discount res true")
                    res = true;
                    break;
                }
        ...
        get_available_rewards: function(){
            ...
            var gift_reward_set = this.has_gift_reward();
            console.log("discount_reward_sett=", discount_reward_set);
            for (var i = 0; i < this.pos.loyalty.rewards.length; i++) {
                ...
                if (reward.minimum_points > this.get_spendable_points()) {
                    console.log('min point> spendable');
                    continue;
                } else if(reward.type === 'gift'  &&
                          (gift_reward_set || reward.point_cost > this.get_spendable_points())) {
                    console.log('gift and point cost> spendable');
                    continue;
                } else if(reward.type === 'discount' &&
                         (discount_reward_set || reward.point_cost > this.get_spendable_points())){
                    console.log('disc and disc reward or min point> spendable');
                    continue;
                }
                console.log("rewrd push", rewards);
                rewards.push(reward);
            }
        ...
        export_as_JSON: function(){
            ...
            json.loyalty_won = this.get_won_points();
