...
class BankBookReportWizard(models.TransientModel):
    _name = "bank.book.report"

    period_start = fields.Date("Period From")
    period_stop = fields.Date("Period To")
    partner_id = fields.Many2one('res.partner', "Partner")
    account_id = fields.Many2one('account.account', "Account", domain=[('is_bank','=',True)])

    @api.multi
    def print_bank_book_report(self):
        data = {
        ...
        return self.env['report'].get_action([], 'report_bank_book.bank_book_report_pdf', data=data)


class ReportAdvPayment(models.AbstractModel):
    _name = 'report.report_bank_book.bank_book_report_pdf'

    def find_opening_balance(self, period_start, period_stop, partner_id, account_id):
        cr = self.env.cr
        accounts = self.env['account.account'].search([('is_bank', '=', True)])
        account_ids = []
    ...
    @api.model
    def get_bank_book_details(self, period_start=False, period_stop=False, partner_id=False, account_id=False):
        dom = [
            ('move_id.state', '=', 'posted'),
            ('account_id.is_bank', '=', True),
        ]
        if period_stop:
        ...
        bank_book_records = self.env['account.move.line'].search(dom,)
        debit_sum = 0.0
        credit_sum = 0.0
        debit_opening = 0.0
        credit_opening = 0.0
        ...
        current_balance = 0.0
        for bank_book in bank_book_records:
            debit_sum += bank_book.debit
            credit_sum += bank_book.credit
            current_balance += (bank_book.debit - bank_book.credit)
        closing_balance = opening_balance + current_balance
        return {
            'bank_book_records': sorted(bank_book_records, key=lambda l: l.date),
    ...
    def render_html(self, docids, data=None):
        data = dict(data or {})
        data.update(self.get_bank_book_details(data['period_start'],
                                                 data['period_stop'],
                                                 data['partner_id'],
                                                 data['account_id'],
                                                 ))
        return self.env['report'].render('report_bank_book.bank_book_report_pdf', data)
