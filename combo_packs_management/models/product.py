...
class ComboLines(models.Model):
    ...
    type = fields.Selection([('unit_price', 'Unit Price'),
                             ('total_price', 'Total Price')], 'Price Method', required=True,
                            store=True, default='unit_price')
    price = fields.Float('Offer Price Per Unit', store=True)
    total = fields.Float('Total Price', store=True)
    subtotal = fields.Float('Sub Total', compute='_get_subtotal', store=True)

    @api.multi
    @api.depends('price', 'total', 'count')
    def _get_subtotal(self):
        for line in self:
            if line.type == 'unit_price':
                line.total = line.count * line.price
                line.subtotal = line.count * line.price
            else:
                line.price = line.total / line.count
