...
$.fn.fullCalendar2 = function(options) {
        ...
        $.each(
			{
				...
				contextmenu: function (seg, ev) {
					view.trigger('stateCtrl', this, seg.event, ev);
					return false
				}
			},
			function(name, func) {
...
TimeGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		state_scheduler_color = "fc-bg state_appt_booked";
		if (event.state_appt == 'Booked'){
		    state_scheduler_color = "fc-bg state_appt_booked";
		}
		if (event.state_appt == 'Confirmed'){
		    state_scheduler_color = "fc-bg state_appt_confirmed";
		}
		if (event.state_appt == 'No Response'){
		    state_scheduler_color = "fc-bg state_appt_no_response";
		}
		if (event.state_appt == 'Cancelled'){
		    state_scheduler_color = "fc-bg state_appt_cancelled";
		}
		if (event.state_appt == 'Executed'){
		    state_scheduler_color = "fc-bg state_appt_executed";
		}
		return '<a class="' + classes.join(' ') + '"' +
			(event.url ?
				' href="' + htmlEscape(event.url) + '"' :
			...
			'>' +
				'<div class="fc-content">' +
					...
					(event.title ?
						'<div class="fc-title">' +
						'' + htmlEscape(event.partner_id[1]) + ': ' +
							htmlEscape(event.title) +
						'</div>' :
						''
						) +
				'</div>' +
				'<div class="' + state_scheduler_color + '"/>' +
...
function Calendar_constructor(element, overrides) {
	...
	// NOTE forced arg is used for forced rerender after update scheduler resources
	function renderView(viewType, forced) {
		ignoreWindowResize++;
...
Calendar.defaults = {
	...
	nextDayThreshold: '10:00:00', // 10am
