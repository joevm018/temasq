odoo.define('calendar_scheduler.GrouppedCalendar', function(require){
    "use strict";

    /*---------------------------------------------------------
     * OpenERP web_calendar
     * NOTE Copied from web.calendar
     * This class uses for old functionality API of fullcalendar 2
     * i.e. this module upgrade CalendarView to use fullcalendar 2
     * & add resource view for agendaDayView by default
     *
     * It is a user task to provide resources Ids to the view
     * By default, resources View used for agendaDayView only
     *---------------------------------------------------------*/
    ...
    var widgets = require('calendar_scheduler.widgets');
    ...
    var CalendarView2 = View.extend({
        ...
        events: {
            ...
            'click .o_calendar_refresh': 'calendar_refresh',
        },
        icon: 'fa-refresh',
        quick_create_instance: widgets.QuickCreate,
        template: "CalendarView",

        calendar_refresh: function(){
            var input_value = false;
            var text;
            var domain = [];
            var label = $('.o_searchview_facet_label').text().trim();
            if(label == 'Employee'){
                input_value = $('.o_facet_values span');
                for (var i = 0; i < input_value.length; i++) {
                    text = input_value[i].textContent.trim();
            ...
            if (domain) {
                local_storage.setItem('searchname', domain);
            }
            location.reload();
        },
        init: function () {
            ...
            // TODO
             this.resource_id = attrs.staff_assigned_id;
        ...
        willStart: function () {
            var self = this;
            var cal_extras_def = new Model('calender.config').call("fetch_calendar_extras", []);
            ...
            return $.when(write_def, create_def, cal_extras_def, this._super()).then(function (write, create, extras) {
                    self.write_right = write;
                    self.create_right = create;
                    if (extras) {
                        self.schedule_start =
                            extras[0].schedule_start ? extras[0].schedule_start : "09:00";
                        self.schedule_end =
                            extras[0].schedule_end ? extras[0].schedule_end : "23:55";
                        self.services = extras[1];
                        self.config_staff_type = extras[3];
                        var temp = extras[2];
                        $.each(temp, function(p) {
                            self.customer_by_id[temp[p]['id']] = temp[p];
                            self.customer_by_name.push({
                                label:temp[p]['name'],
                                value:temp[p]['id']
                            });
                        });
...
//            var self = this;
            ...
            return this._super();/*.then(function () {
                self.do_search([], self.dataset.context,[]);
                *//*self.$calendar.fullCalendar2('addEvent', {
                'start': "2018-11-6T01:30",
                'end': "2018-11-6T02:30",
                'title': "gggggg",
...
//                'attendees':attendees,
        ...
        get_fc_init_options: function () {
            ...
            return $.extend({}, get_fc_defaultOptions(), {
                ...
                minTime: self.schedule_start,
                maxTime: self.schedule_end,
                ...
                eventDrop: function (event, _delta, _revertFunc) { //_day_delta, _minute_delta, _all_day, _revertFunc) {
                            var restrict_appt = new Model('calender.config').call('restrict_done_appt_modif_non_admin',
                            [event.id, event.state])
                            .then(function (restrict_modif) {
                                  if (restrict_modif == false){
                                    if (!confirm("Are you sure about this change?")) {
                                        ...
                                        var temp = new Date(data['procedure_start']);
                                        data['procedure_start'] = temp.toUTCString();
                                        if (data['procedure_stop']) {
                                            temp = new Date(data['procedure_stop']);
                                            data['procedure_stop'] = temp.toUTCString();
                                        }
                                        self.proxy('update_record')(event._id, data); // we don't revert the event, but update it.
                                    }
                                }
                                if (restrict_modif == true){
                                    _revertFunc();
                                }
                            });
                },
                eventResize: function (event, _delta, _revertFunc) { //_day_delta, _minute_delta, _all_day, _revertFunc) {
                    var restrict_appt = new Model('calender.config').call('restrict_done_appt_modif_non_admin',
                            [event.id, event.state])
                            .then(function (restrict_modif) {
                                  if (restrict_modif == false){
                ...
                stateCtrl: function (ev1, ev2) {
                    var curr_state = ev1.state;
                    var states_list = [];
                    var new_state_sel;
                    if (curr_state == 'draft') {
                         states_list.push('cancel');
                    }
                    else {
                        return false
                    }
                    var opt = {
                        style_val: 'left:'+ (ev2.pageX - 180)+'px;top:'+(ev2.pageY - 150)+
                            'px;width: 300px; height: 170px;',
                        states_list: states_list,
                    };
                    var sched_el = $(QWeb.render(
                        'calendarModal2',
                        opt
                    ));
...
//                        var new_state = sched_el.find('.state_ctrl').val();
                        var new_state = 'cancel'
                        var new_color = ev1.color;
                        ev1.state = new_state;
//                        self.trigger_up('updateState', ev1);
                        console.log(ev1)
                        console.log(ev1.statement_ids.length)
                        if(ev1.statement_ids.length>0){
                            alert('You Cannot cancel orders that is already paid')
                            sched_el.find(".close_btn").click()
                            return false;
                        }
                        var action_cancel = new Model('pos.order').call('action_cancel_order_js',
                         [ev1.order_id, new_state]).then(function (warning_here) {
                              console.log(warning_here)
                        });
...
//                context.$calendar.fullCalendar2('gotoDate', moment(curDate).add(1, 'day'));
        ...
        init_calendar: function() {
            ...
            var self = this;
            var domain = [['is_beautician', '=', 1]]
            var lo_st = local_storage.getItem('searchname');
            if (lo_st) {
                var text = lo_st.split(',')
                if (text.length > 1) {
                    for (var i = 0; i < text.length; i++) {
                        domain.push(["name", "ilike", text[i]]);
                    }
                    var lst_or = []
                    for (var j = 0; j < domain.length - 1; j++) {
                        lst_or.push('|');
                    }
                    domain = lst_or.concat(domain);
                }
                else {
                    domain.push(["name", "ilike", lo_st]);
                }
                local_storage.removeItem('searchname')
            }
            /*for (var i = 0; i < input_text.length; i++) {
                if (input_text.length > 1) {
                    domain.push(["name", "ilike any", input_text[i].trim()]);
                }
                else {
                    domain.push('|', ["name", "ilike", input_text[i].trim()]);
                }
            }*/

            var res_defs = new Model('hr.employee').query(["name", "id", "is_beautician", "service_ids"]).filter(domain).all().then(function(res) {
                var options_def = self.get_fc_init_options();
                _(res).each(function (resource) {
...
//                    title: "Others"
//                });
                options_def['resources'] = res;
                self.$calendar.fullCalendar2(options_def);
                return;
            });

            return $.when.apply($, defs, res_defs);
        },
...
//            var date_delay = evt[this.date_delay] || 1.0,
            ...
            var the_title2 =  the_title;
            var temp = the_title.split(',');
            // TODO resourceId = self.fields[fieldName]
             var resourceId = the_title2.substr(0,1);
            var r = {
                'start': moment(date_start).format("YYYY-MM-DDTHH:mm:ss"),
                'end': moment(date_stop).format("YYYY-MM-DDTHH:mm:ss"),
                'title': temp[0],
                'attendee_avatars': the_title_avatar,
                //'allDay': (this.fields[this.date_start].type == 'date' || (this.all_day && evt[this.all_day]) || false),
                'allDay': false,
                ...
                'state': evt.state,
                'checkin': evt.checkin,
                'state_appt': evt.state_appt,
                'attendees':attendees,
                'partner_id':evt.partner_id,
                'statement_ids':evt.statement_ids,
                'order_id':evt.order_id,
                'resourceId': evt['staff_assigned_id'] ? evt['staff_assigned_id'][0] : -1
            };
        ...
        get_event_data: function(event) {
            ...
            if (event.allDay) {
                ...
                diff_seconds = Math.round((event_end.toDate().getTime() - event.start.toDate().getTime()) / 1000);
            }
            ...
            data['staff_assigned_id'] = event.resourceId;
            return data;
        },
        ...
        _do_search: function(domain, context, _group_by) {
            ...
            this.event_source = {
                ...
                events: function(start, end, tz, callback) {
                    ...
                    self.dataset.read_slice(_.keys(self.fields), {
                        ...
                        if (self.current_start.toDate().getTime() != start.toDate().getTime() ||
                        self.current_end.toDate().getTime() != end.toDate().getTime()) {
            ...
            setTimeout(function(){
                self.$calendar.fullCalendar2('addEventSource', self.event_source);
            }, 1000);
        },
        /**
...
//            extend_domain.push(['state', '!=', 'cancel'])
            //                    event_domain = new data.CompoundDomain(
//                                    event_domain,
//                                    [['state', '=', 'cancel']]
//                                );
    ...
    core.view_registry.add('scheduler', CalendarView2);
