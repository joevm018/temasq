...
class PosMakePayment(models.TransientModel):
    ...
    def type_credit(self):
        credit = self.session_id.config_id.journal_ids.filtered(
            lambda journal: journal.type == 'bank' and journal.is_pay_later)
        if credit:
            self.journal_id = credit[0]
        if not credit:
            raise UserError('Please define Credit journal')
        return {
            "type": "ir.actions.do_nothing",
        }
